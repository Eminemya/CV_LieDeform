...
function y=see3(tt,dd,ind , name,ff)
...
eval(['a=reshape(' name '(ind(j+cc*10),:,mid),ll,ll)'';'])
imagesc(a)
axis off
%title(num2str(dd(j)))
...
eval(['a=reshape(' name '(dd{j+cc*10}(jj),:,tt{j+cc*10}(jj)),ll,ll)'';'])
%{
...
title(['rot ' num2str(rot(tt{j+cc*10}(jj)))])
axis off
