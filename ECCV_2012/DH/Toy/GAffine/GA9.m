...
%initialization:
c_thres=r_thres*1.5;
g_thres=lc2*0.03; %around 30 clusters
lg=1;
while lg<10 
if lg<10
   c_thres=c_thres-0.01;
end
...
count=1;
while length(rest)>0
[aa,bb]=max(sum(dis(rest,rest)<c_thres,2));
pre_center(count)=rest(bb);
pre_ss(count)=sum(dis(rest(bb),:)<c_thres);
ind=[find(dis(rest(bb),rest)<c_thres)];
rest(ind)=[];
count=count+1;
%[count,length(ind)]
end
...
%clean up
pre_center(pre_ss<g_thres)=[];
pre_ss(pre_ss<g_thres)=[];
lg=length(pre_center);
[digit,lg,c_thres]
end


[label, center] = kmedoids(dis, lg,pre_center);
    ...
    for i=1:length(lan)
        ...
        for jj=1:length(cindex2{lan(i)})
          ...
          % can't concatenate ... should apply in sequence  
            ...
            curr3=curr3+1;
            for k=cindex{j}
                strain(curr3,:)=Trans2(ntrain(k,:),ntrain(kk,:),trans_ind(:,[t,tran(i)]));            
                curr3=curr3+1;
            end
        end
...
eval(['save data/45_dis' num2str(digit) '  dis_train strain land1 land2 land3']);
eval(['save data/45_alltrain' num2str(digit)])
...
for tt=0:9
eval(['load data/45_test' num2str(tt)]);
...
for tt=0:9
    eval(['load data/45_test' num2str(tt)]);
    for ttt=1:10
        [trans,ind,dis]=Align3(aligned(ttt,:),newtimgs,2,1);
        for i=1:size(newtimgs,1)
...
eval(['save data/45_test_2train' num2str(digit) '  timg_a dist_a']);
