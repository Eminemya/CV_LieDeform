...
%dw: no scale so far
%    need hard range restriction for 
...
if len>=3
  theta=transVec(3)*1/10;
  rotMat(1:2,1:2)=[cos(theta) -sin(theta); sin(theta) cos(theta)];
end

if len>=5
  xScaleMat(1,1)=exp(0.5*transVec(4));
  yScaleMat(2,2)=exp(0.5*transVec(5));
end

if len>=7
  xShearMat(1,2)=0.5*transVec(6);
  yShearMat(2,1)=0.5*transVec(7);
end
...
if len<=5
  ...
  xfrm=yShearMat*xShearMat*rotMat*ymat*xmat;
  return;
end
