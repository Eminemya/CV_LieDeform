...
/*2) IDM Algo*/
  ...
  double best_dis,*z;
  double temp;
  int x,y,xx,yy,z1,z2,ww=30;

  int warprange=3;
/* 3) Return distance */
plhs[0] =mxCreateDoubleMatrix(6,row1, mxREAL);
z = mxGetPr(plhs[0]);

for(z1=0;z1<row1;z1++){
z[z1*6]=DBL_MAX;z[z1*6+1]=DBL_MAX;z[z1*6+2]=DBL_MAX;
for(z2=0;z2<row2;z2++){    
    dis=0;
   ...
   if(dis<z[z1*6]){
     ...
     z[z1*6+4]=z[z1*6+3];
     z[z1*6+3]=(double)z2;     
    }else if(dis<z[z1*6+1]){
     z[z1*6+2]=z[z1*6+1];
     z[z1*6+1]=dis;
     z[z1*6+5]=z[z1*6+4];
     z[z1*6+4]=(double)z2;     
    }else if(dis<z[z1*6+2]){
     z[z1*6+2]=dis;
     z[z1*6+5]=(double)z2;
    }
    }
